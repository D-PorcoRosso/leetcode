...
public class Solution {
    public void gameOfLife_2021_250_inplace(int[][] board) {
        for ( int i = 0 ; i < board.length ; i++ ) {
            for ( int j = 0 ; j < board[0].length ; j++ ) {
                int liveCount = isLive_2021_250_inplace(board, j, i);
                if (board[i][j] == 1) {
    ...
    private int isLive_2021_250_inplace(int[][] board, int x, int y) {
        int liveCount = 0;
        ...
        if(x-1 >= 0 && y-1 >= 0 && (board[y-1][x-1] >> 1) == 1) {
            liveCount++;
        }
        if(x-1 >= 0 && y+1 < board.length && board[y+1][x-1] == 1) {
            liveCount++;
        }
        if(x+1 < board[0].length && y-1 >= 0 && (board[y-1][x+1] >> 1) == 1) {
            liveCount++;
        }
